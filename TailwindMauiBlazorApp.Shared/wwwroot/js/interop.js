...
//export function enableDesktopSwipeScroll(containerId) {
//    const container = document.getElementById(containerId);
//    if (!container || 'ontouchstart' in window) return; // skip mobile devices
...
//    // Save places data for reuse
//    globeData = places && places.length ? places : getDefaultPlaces();
...
//            el.style.pointerEvents = 'none';  // disable pointer events on the wrapper container

//            const wrapper = document.createElement('div');
//            wrapper.className = 'marker-wrapper';
//            wrapper.style.pointerEvents = 'auto';  // enable pointer events for click
//            wrapper.style.display = 'inline-block';
...
//            wrapper.insertAdjacentHTML('beforeend', markerSvg);

//            // Remove hover handlers
//            // Add click handler to show popup
//            wrapper.onclick = (e) => {
//                e.stopPropagation();  // prevent click from propagating to globe (if needed)
//                showMarkerPopup(wrapper, d);
//            };

//            // Redirect wheel events to the globe canvas
//            wrapper.addEventListener('wheel', (event) => {
//                event.preventDefault(); // Prevent the marker from handling it
//                const canvas = globeContainer.querySelector('canvas');
//                if (canvas) {
//                    const newEvent = new WheelEvent(event.type, event);
//                    canvas.dispatchEvent(newEvent);
...
//            // Redirect pointermove events for drag to globe
//            wrapper.addEventListener('pointermove', (event) => {
//                const canvas = globeContainer.querySelector('canvas');
//                if (canvas) {
//                    const newEvent = new PointerEvent(event.type, event);
...
//    // Add global click listener to hide popup when clicking outside
...
//export function fitGlobe() {
//    if (!globeContainer || !globeInstance || !globeData || globeData.length === 0) return;

//    const rect = globeContainer.getBoundingClientRect();
//    globeInstance.width(rect.width);
//    globeInstance.height(rect.height);

//    const lats = globeData.map(d => d.lat);
//    const lngs = globeData.map(d => d.lng);
//    const minLat = Math.min(...lats);
//    const maxLat = Math.max(...lats);
//    const minLng = Math.min(...lngs);
//    const maxLng = Math.max(...lngs);
...
//    const fitAltitude = Math.tan((paddedSpanDeg * Math.PI) / 360) * 2.2;
//    const verticalOffsetDeg = (maxLat - minLat) * 0.0;

//    setTimeout(() => {
//        globeInstance.pointOfView({
//            lat: avgLat + verticalOffsetDeg,
//            lng: avgLng,
//            altitude: fitAltitude
//        }, 2500);
//    }, 500);
...
//            Latitude: ${data.lat.toFixed(4)}<br>
//            Longitude: ${data.lng.toFixed(4)}<br>
//            Color: ${data.color}
//        </div>
//    `;

//    popup.classList.remove('hovered'); // Reset
...
//    // Close button
//    document.getElementById("closePopupBtn")?.addEventListener("click", hideMarkerPopup);

//    // Hover logic
//    popup.onmouseenter = () => {
//        popupHovered = true;
//        popup.classList.add('hovered'); // enable pointer-events
//    };
//    popup.onmouseleave = () => {
//        popupHovered = false;
//        popup.classList.remove('hovered');
//        setTimeout(() => {
//            if (!popupHovered) hideMarkerPopup();
//        }, 300);
//    };
//}
...
//    popup.style.display = 'none';
//    popup.style.pointerEvents = 'none'; // disable interaction so zoom passes through
//}